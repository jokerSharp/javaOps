...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<Integer, Integer> caloriesByDayMap = new HashMap<>();

        for (UserMeal userMeal : meals) {
            Integer day = userMeal.getDateTime().getDayOfMonth();
            Integer calories = userMeal.getCalories();
            caloriesByDayMap.merge(day, caloriesByDayMap.getOrDefault(day, calories), (k, v) -> v + calories);
        }
            ...
            if (userMeal.getDateTime().toLocalTime().isAfter(startTime) && userMeal.getDateTime().toLocalTime().isBefore(endTime)) {
                userMealWithExcessList.add(
                        new UserMealWithExcess(userMeal.getDateTime(),
                                ...
                                (caloriesByDayMap.get(userMeal.getDateTime().getDayOfMonth()) > caloriesPerDay))
                );
            }
        });
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(meal -> caloriesByDayMap.merge(
                meal.getDateTime().getDayOfMonth(),
                caloriesByDayMap.getOrDefault(meal.getDateTime().getDayOfMonth(), meal.getCalories()),
                (k, v) -> v + meal.getCalories()));
